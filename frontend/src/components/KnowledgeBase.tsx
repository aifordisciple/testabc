...
export default function KnowledgeBase() {
  ...
  const searchMutation = useMutation({
    ...
    onError: (err: any) => toast.error(`Search Error: ${err.message}`)
  });
  ...
  const importMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success(`Successfully imported ${selectedDataset?.accession} to your project!`);
        ...
        <div className="space-y-6">
          {searchMutation.isPending && (
            <div className="flex flex-col items-center justify-center py-12 gap-4">
               <span className="relative flex h-8 w-8"><span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span><span className="relative inline-flex rounded-full h-8 w-8 bg-blue-500"></span></span>
               <p className="text-gray-400 animate-pulse">Running semantic vector search...</p>
            </div>
          )}

          {!searchMutation.isPending && hasSearched && results.length === 0 && (
             <div className="text-center py-12 text-gray-500">No matching datasets found.</div>
          )}

          {!searchMutation.isPending && results.map((dataset) => (
            <div key={dataset.id} className="bg-[#0f1218] border border-gray-800 rounded-xl p-6 hover:border-gray-600 transition-all shadow-md group">
      ...
      {isModalOpen && selectedDataset && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100]">
          <div className="bg-gray-900 p-6 rounded-xl border border-gray-700 w-[28rem] shadow-2xl">
            ...
            {projects.length === 0 ? (
                <div className="text-yellow-500 text-sm mb-4">You don't have any projects yet. Please create one first.</div>
            ) : (
                <select 
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-white mb-6 outline-none focus:border-blue-500"
            ...
            <div className="flex justify-end gap-3">
              ...
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-500 text-white px-5 py-2 rounded-lg text-sm font-medium disabled:opacity-50 transition-colors"
              >
